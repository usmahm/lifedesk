...
- [x] **Focus mode** — `/focus`, a route outside `AppShell` so the rail, tab bar and timer bar are _absent_ rather than hidden. Shows the running session's task, the clock, editable notes, estimate vs tracked, and today's sessions on that task
  ...
  - **Works alongside the floating window**, togglable from inside Focus. PiP ownership moved to a root provider so the window survives navigating between the shell and Focus — it previously lived in `TimerBar`, which unmounts on `/focus`, so popping out and then focusing closed the very window you had just opened
  - **`usePomodoro` takes `drive`, and exactly one mounted component passes it.** Its side effects are not idempotent — two drivers would count one phase twice and race each other's writes. `PomodoroEngine` is that driver, mounted once per authed layout; the bar, Focus and the floating window all read
